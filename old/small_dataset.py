...
def map_files(csv_data):
	...
	for i in tqdm(csv_data):
		...
		with open(i[0], 'r') as mfile:
			data = mfile.read()
			if(i[1] == "Dark Hotel" or i[1] == "APT 10" ):
...
def open_csv(fname, base_path):
	...
	csv_data = [i.strip('\n').split(',') for i in data]
	for i in csv_data:
		if 100 < get_file_size(base_path+i[0]) < 1000000:
...
if __name__ == "__main__":
	...
	split_to_file("test_set_small.csv", test_files)
	split_to_file("train_set_small.csv", train_files)
	...
	to_config("tokenized_with_args_small.config", train_len,test_len,max_len_args)
	to_config("tokenized_with_instructions_small.config", train_len, test_len,max_len_inst)

	print("[* ARG TRAIN DATA TO FILE *]")
	for index, i in enumerate(tqdm(train_data_args)):
			write_to_data_file("args_small",i,str(index),test=False,max_len=max_len_args)
	print("[* ARG TEST DATA TO FILE *]")
	for index, i in enumerate(tqdm(test_data_args)):
		write_to_data_file("args_small",i,str(index), test=True,max_len=max_len_args)

	print("[* INSTR TRAIN DATA TO FILE *]")
	for index, i in enumerate(tqdm(train_data_instr)):
			write_to_data_file("instr_small",i,str(index),test=False,max_len=max_len_inst)
	print("[* INSTR TEST DATA TO FILE *]")
	for index, i in enumerate(tqdm(test_data_instr)):
		write_to_data_file("instr_small",i,str(index), test=True,max_len=max_len_inst)
